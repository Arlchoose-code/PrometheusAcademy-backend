...
func RegisterAuthRoutes(router *gin.RouterGroup, db *gorm.DB, cfg config.Config) {
	...
	auth.POST("/otp/verify", authController.VerifyAuthOTP)
	auth.POST("/otp/resend", authController.ResendAuthOTP)
	auth.POST("/password-reset/request", authController.RequestPasswordReset)
