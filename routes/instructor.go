...
func RegisterInstructorRoutes(router *gin.RouterGroup, db *gorm.DB, cfg config.Config) {
	...
	instructor.GET("/media", controller.ListMedia)
	instructor.POST("/media", controller.CreateMedia)
	instructor.DELETE("/media/:id", controller.DeleteMedia)
	...
	instructor.GET("/reviews", controller.ListReviewQueue)
	instructor.PUT("/assignment-submissions/:id/review", controller.ReviewAssignmentSubmission)
	instructor.PUT("/quiz-submissions/:id/review", controller.ReviewQuizSubmission)
